...
function breakcrumb(string $title, string $icon)
{
  return '
   <!-- breakcrumb -->
  ...
  if (!empty($salaires)) {
    ...
    foreach ($salaires as $data) {
      ...
      <tr>
        ...
        <td class="text-center" data-titles="<?= $data['user_confirm'] ?>"><?= checkStateDepense($data['etat_salaire']) ?>
        </td>
        <td class="table_button">

          <div class="dropdown">
            <button type="button" <?= ($data['etat_salaire'] == 1 ? 'disabled' : '') ?>
              class="btn btn-dark btn-sm dropdown-toggle" data-toggle="dropdown">

            </button>

            <div class="dropdown-menu">

              <button class="dropdown-item btn_confirm_salaire" data-salaire="<?= crypter($data['code_salaire']) ?>">
                ✅ Confirmer</button>
              <button class="dropdown-item frmModifierSalaire" data-salaire="<?= crypter($data['code_salaire']) ?>">
                📝 Modifier</button>
              <button class="dropdown-item btn_delete_salaire" data-salaire="<?= crypter($data['code_salaire']) ?>"> ❌
                Annuler</button>
            </div>
          </div>
        </td>
          ...
          <div class="dropdown">
            ...
            <div class="dropdown-menu">
              ...
              <?php if (!empty($data['token']) && $data['etat_user'] == 0) : ?>
                <button data-user="<?= crypter($data['code_user']) ?>" class="dropdown-item btn_send_mail"> 📧 Renvoyer
                  mail </button>
              <?php endif; ?>
        ...
        "> 👁‍🗨 Voir </a>
              ...
              <?php if (empty($data['token'])) : ?>
                <?php if ($data['etat_user']) : ?>
                  ...
                  <button class="dropdown-item btn_disable_user" data-user="<?= crypter($data['code_user']) ?>"> ❌
                    Desactiver compte</button>
                <?php else : ?>
                  <button class="dropdown-item btn_enable_user" data-user="<?= crypter($data['code_user']) ?>"> ✅ Activer
                    compte</button>
  ...
  if (!empty($categories)) {
    ...
    foreach ($categories as $data) {
      ...
      <tr>
        ...
        <td class="table_button">
          <button id="frmModifierCategorie<?= $i ?>" type="button"
            class="btn btn-primary btn-sm mr-2 frmModifierCategorie" data-categorie="<?= ($data['code_categorie']) ?>">
            <i class="fa fa-edit"></i> Modifier
          </button>

          <button id="categorieDeleteCategorie<?= $i ?>" type="button"
            class="btn btn-danger btn-sm categorieDeleteCategorie" data-categorie="<?= ($data['code_categorie']) ?>">
  ...
  if (!empty($produits)) {
    ...
    foreach ($produits as $data) {
      ...
      <tr>
        ...
        <!-- Actions -->
        <td class="table_button">
          <button id="frmModifierProduit<?= $i ?>" type="button" class="btn btn-primary btn-sm mr-2 frmModifierProduit"
            data-produit="<?= $data['code_produit'] ?>">
            <i class="fa fa-edit"></i> Modifier
          </button>

          <button id="produitDeleteProduit<?= $i ?>" type="button" class="btn btn-danger btn-sm produitDeleteProduit"
            data-produit="<?= $data['code_produit'] ?>">
            <i class="fa fa-trash"></i> Supprimer
          </button>
        </td>
  ...
  if (!empty($marks)) {
    ...
    foreach ($marks as $data) {
      ...
      <tr>
        ...
        <td class="table_button">
          <button id="frmModifiermark<?= $i ?>" type="button" class="btn btn-primary btn-sm mr-2 frmModifierMark"
            data-mark="<?= ($data['code_mark']) ?>">
            <i class="fa fa-edit"></i> Modifier
          </button>

          <button id="markDeleteMark<?= $i ?>" type="button" class="btn btn-danger btn-sm markDeleteMark"
            data-mark="<?= ($data['code_mark']) ?>">
            <i class="fa fa-trash"></i> Supprimer
          </button>
        </td>
  ...
  if (!empty($unites)) {
    ...
    foreach ($unites as $data) {
      ...
      <tr>
        ...
        <td class="table_button">
          <button id="frmModifierUnite<?= $i ?>" type="button" class="btn btn-primary btn-sm mr-2 frmModifierUnite"
            data-unite="<?= ($data['code_unite']) ?>">
            <i class="fa fa-edit"></i> Modifier
          </button>

          <button id="uniteDeleteUnite<?= $i ?>" type="button" class="btn btn-danger btn-sm uniteDeleteUnite"
            data-unite="<?= ($data['code_unite']) ?>">
            <i class="fa fa-trash"></i> Supprimer
          </button>
        </td>
...
?>
  ...
  <div class="col-md-4">
    <div class="card bg-grad-dark">
      <div class="card-body text-center ">
        <div class="card-title">
          <h4 class="text-white"> <a class="text-white" href="<?= route('facture.attente') ?>"> ❓ Total Non regles
              <span class="text-white"
                id="countReservationNonRegler">(<?= $data['countReservation'] ?>)</span></a> </h4>
        </div>
        <div class="card-title">
  ...
  <div class="col-md-6">
    <div class="card bg-grad-dark">
      <div class="card-body text-center">
        <div class="card-title">
          <h4 class="text-white"> <a class="text-white" href="<?= route('reservation.annuler') ?>">❌ Reservations
              ennulées <span class="text-white"
                id="countReservationEnnuler"><?= $data['countReservationEnnuler'] ?></span> </a> </h4>
        </div>
        <div class="card-title">
  ...
  <div class="col-md-6">
    <div class="card bg-grad-dark">
      <div class="card-body text-center">
        <div class="card-title">
          <h4 class="text-white"> <a class="text-white" href="<?= route('service.annuler') ?>">❌ Services ennulés
              <span class="text-white" id="countServiceEnnuler"><?= $data['countServiceEnnuler'] ?></span>
            </a> </h4>
        </div>
        <div class="card-title">
          ...
          <div class="dropdown">
            ...
            <div class="dropdown-menu">
              <a class="dropdown-item"
                href="<?= route('reservation.details', ['code' => $data['code_reservation']]) ?>"> <i
              ...
              <?php if ($data['date_entree'] >= date('Y-m-d') && $data['etat_reservation'] == 0 && $state != STATUT_RESERVATION[2]) : ?>
                ...
                <?php if ($data['montant_services'] <= 0) : ?>
                  ...
                  <button class="dropdown-item btn_annuler_reservation"
                    data-reservation="<?= crypter($data['code_reservation']) ?>"> <i class="fa fa-times text-danger">
                    </i> &nbsp;
                    Ennuler</button>
                <?php endif; ?>
              <?php endif; ?>
              ...
              <?php if ($data['date_sortie'] >= date('Y-m-d') && $state != STATUT_RESERVATION[2]) : ?>
                ...
                <button class="dropdown-item btn_modal_service_reservation"
                  data-reservation="<?= crypter($data['code_reservation']) ?>"> <i class="fa fa-check text-success">
                  </i> &nbsp;
                  Operation</button>
              <?php endif; ?>
              <?php if ($state != STATUT_RESERVATION[2]) : ?>

                <button class="dropdown-item btn_modal_confirme"
                  data-reservation="<?= crypter($data['code_reservation']) ?>">
                  <i class="fa fa-check text-success"> </i> &nbsp; Regler facture</button>
                <a class="dropdown-item" target="_blank"
                  href="<?= route('facture', ['code' => $data['code_reservation']]) ?>">
...
function chargerStateSouscriptionForComercial(){
    ...
    $stats = (new ClientModel())->getStatsInscriptionsForCommercial(auth()->user('etablissement_code'), auth()->user('annee_code'), auth()->user('zone_code'),auth()->user('id'));
...
function chargerStateSouscription($dateStart,$dateEnd){
    ...
    $stats = (new ClientModel())->getStatsInscriptionsForCommercial(auth()->user('etablissement_code'), auth()->user('annee_code'), auth()->user('zone_code'),auth()->user('id'),$dateStart,$dateEnd);
    var_dump($stats);
...
// auth()->user('etablissement_code'), auth()->user('annee_code'), auth()->user('zone_code'),auth()->user('id')
// var_dump($listeSoucription);
// return;
  $output = '';
...
function searchFormTable($title)
{
  ...
  <div class="row d-flex justify-content-between  items-center m-2 p-1">
    ...
    <div class="col-md-6">
      <h4 class="text-bold"> <span style="cursor: pointer;" class="synchroniser"><i class="fa fa-redo"></i></span>
        &nbsp; <?= $title ?>
        [ <span id="counter_items">0</span> ] </h4>
    </div>
...
?>
  <div class="row d-flex justify-content-between  items-center m-2 p-1">
    ...
    <div class="col-md-6">
      <h4 class="text-bold"> <span style="cursor: pointer;" class="synchroniser"><i class="fa fa-redo"></i></span>
        &nbsp; <?= $title ?>
        [ <span class="counter_items">0</span> ] </h4>
    </div>
    <div class="col-sm-12 col-md-4">
      ...
      <div class="d-flex justify-content-end items-center">
        ...
        <div class="form-group">
          <button type="button" data-periode="<?= $periode ?>" class="btn btn-default"
            id="<?= $searchSelector ?>"> <i class="fa fa-print"></i> IMPRIMER </button>
