...
class FinanceController extends MainController
{
    ...
    {
        $this->view('etudiants/inscription', ['title' => "Inscription"]);
    }
    ...
    public function GetListeInscription()
    {
        ...
        $data = $this->financeService->depenseDataService($depenseList);
    ...
    {
        $typeDepenses = $this->financeModel->getAllTypeDepenses(Auth::user('etablissement_code'));
        if (empty($typeDepenses)) Response::error('Désolé, aucune année enregistrée!');

        $output = $this->financeService->depenseAddModalService($typeDepenses);
    ...
    public function modalUpdatedDepense()
    {
        ...
        // $users = getAllusers();
        $depense = $this->financeModel->getSingledepenseByCode($codedepense);
        ...
        $output = $this->financeService->depenseUpdateModalService($depense, $typeDepenses);
    ...
    public function addDepense()
    {
        ...
        $result = $this->financeService->saveDepenseData($_POST);
    ...
    public function updateDepense()
    {
        ...
        $result = $this->financeService->updateDepenseData($_POST);
    ...
    public function changeStatutDepense()
    {
        ...
        if ($this->financeModel->update(TABLES::DEPENSES, 'code_depense', $code_depense, ['statut_depense' => $statut_depense])) Response::success('Statut modifié avec succès', []);
